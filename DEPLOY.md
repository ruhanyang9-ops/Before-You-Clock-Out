...
开启 DeepSeek 真实 AI 和内测口令：

```bash
DEEPSEEK_API_KEY="你的key" AI_MODEL="deepseek-v4-flash" APP_ACCESS_PASSWORD="内测口令" node server.js
```

未配置 `DEEPSEEK_API_KEY` 时，应用会自动使用 mock / 本地规则模式；未配置 `APP_ACCESS_PASSWORD` 时，不启用访问保护。
...
Environment=APP_ACCESS_PASSWORD=请替换为内测口令
Environment=DEEPSEEK_API_KEY=请替换为你的key
